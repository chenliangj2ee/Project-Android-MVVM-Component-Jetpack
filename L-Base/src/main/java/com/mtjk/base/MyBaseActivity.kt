...
//        log("MyActivityManager", "启动-----------${javaClass.name}")
    ...
    override fun onResume() {
        ...
        BaseInit.act = this
        mHttpEvent.register()
        TCAgent.onPageStart(this, this.localClassName)
    }
    ...
    override fun onPause() {
        super.onPause()
        mHttpEvent.onPause()
        TCAgent.onPageEnd(this, this.localClassName)
    }
...
//                toast("没有权限")
    ...
    open fun checkUpgrade(loading: Boolean) {
        ...
        initVM(AppModelModel::class.java).check().obs(this) {

            it.y { bean ->
                if (bean != null && bean!!.versionCode > MyApp.getVersionCode(this)) {
                    if (bean.upgradeDescription == null)
                        bean.upgradeDescription = "是否升级？"
                    dialog(bean.upgradeDescription).title("发现最新版本:${bean.versionName}")
                        .n {
                            if (bean.forceUpdate == 1) {
                                finish()
                            }
                        }
                        .y { MyApp.openBrowser(bean.fileUrl) }
                        .show(this)
                } else {
