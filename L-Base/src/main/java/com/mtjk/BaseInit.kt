...
object BaseInit {
    ...
    fun exit() {
        ...
        exitUser.phone = user!!.phone
        exitUser.save()
        BaseInit.act?.goto("base/login")
        XGPushManager.unregisterPush(con, object : XGIOperateCallback {
            override fun onSuccess(data: Any, i: Int) {
                log("TPush", "反注册成功")
            }

            override fun onFail(data: Any, errCode: Int, msg: String) {
                log("TPush", "反注册失败，错误码：$errCode,错误信息：$msg")
            }
        })
    }
