...
import com.mtjk.base.MyDefaultFragment
import com.mtjk.base.R
import com.mtjk.bean.BeanUser
import com.mtjk.obj.ObjectID
...
@BindingAdapter(value = ["load", "radius"], requireAll = false)
fun imageUrl(view: ImageView, url: String?, radius: Int) {
//    view.log("图片加载$radius:$url")
    url?.let {
        if (radius > 0) {
            view.load(it, MyScreen.dip2px(view.context, radius.toFloat()))
...
@BindingAdapter(value = ["bindBackground"])
...
@BindingAdapter(value = ["bindButtonColor"])
...
fun Any.loading(vararg messages: String): Dialog {
    var dialog = Dialog(BaseInit.act)
...
fun Any.messageNotification(id: String, title: String, message: String) {
    log("通知：$id    $title    $message")
    if (MyApp.isAppRunningForeground()) {
//        vibrate()
        return
    }

    val manager =
        BaseInit.act?.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
    var notify: Notification? = null
    var intent = Intent(
        BaseInit.act!!,
        Class.forName("com.tencent.qcloud.tuikit.tuichat.ui.page.TUIC2CChatActivity")
    )
    val param = Bundle()
    param.putInt("chatType", 1)
    param.putString("chatId", id)
    param.putString("chatName", title)
    intent.putExtras(param)
    intent.action = System.currentTimeMillis().toString();

    val pendingIntent3 =
        PendingIntent.getActivity(
            BaseInit.act?.applicationContext,
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        val channel = NotificationChannel("chat", "chat", NotificationManager.IMPORTANCE_HIGH)
    ...
    notify.vibrate = longArrayOf(0, 200, 100, 200)
    notify.defaults = notify.defaults or Notification.DEFAULT_SOUND or Notification.DEFAULT_VIBRATE

    var notifyId = 0
    if (ObjectID.ids.containsKey(id)) {
        notifyId = ObjectID.ids[id]!!
    } else {
        notifyId = ((Math.random() * 1000000) + 1).toInt()
        ObjectID.ids[id] = notifyId
    }
