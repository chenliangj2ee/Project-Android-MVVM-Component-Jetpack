...
public class ThirdPushTokenMgr {
    ...
    public void setPushTokenToTIM() {
        ...
        if (BaseInit.INSTANCE.isUserApp()){
            ...
            if (BrandUtil.isBrandXiaoMi()) {
                v2TIMOfflinePushConfig = new V2TIMOfflinePushConfig(PrivateConstants.EXPERT_XM_PUSH_BUZID, token);
            } else if (BrandUtil.isBrandHuawei()) {
                v2TIMOfflinePushConfig = new V2TIMOfflinePushConfig(PrivateConstants.EXPERT_HW_PUSH_BUZID, token);
            } else if (BrandUtil.isBrandOppo()) {
                v2TIMOfflinePushConfig = new V2TIMOfflinePushConfig(PrivateConstants.EXPERT_OPPO_PUSH_BUZID, token);
            } else if (BrandUtil.isBrandVivo()) {
                v2TIMOfflinePushConfig = new V2TIMOfflinePushConfig(PrivateConstants.VIVO_PUSH_BUZID, token);
            } else {
                return;
