 ...
 * tag==观众-1对1
 */
class GuanZhongActivity() : LiveRoomActivity(), View.OnClickListener,
...
//            userLayout.removeAllViews()
    ...
    fun waitLink() {
        runOnUiThread(Runnable {
            ...
            waitingLinkMicDialog!!.show(this@GuanZhongActivity)
        })
    }
    ...
    override fun onRtmSeatStateChanged(list: List<SeatStateMessageDataItem>) {
        ...
        for (i in list.indices) {
            if ((user!!.userId == list[i].user.userId)) {
                stopWaitLink()
            }
        }
    ...
    override fun onSeatDialogItemClicked(position: Int, operation: SeatItemDialog.Operation) {
        ...
        dialog(message).y {
            ...
            runOnUiThread(Runnable { mSeatLayout!!.updateStates(Live.seats) })
        ...
        if (list.any { user!!.userId == it }) {
            drawInviteDialog=dialog("咨询师向你发送在线共享画板").n("拒绝") {
                Live.sendPeer(
                    this,
                    liveParam!!.userId,
                    SeatInteraction.LIVE_DRAWING_REJECT_INVITE,
                    0,
                    RtmMessageManager.PEER_MSG_TYPE_SEAT,
                    0,
                    object : ResultCallback<Any?> {
